...
m_c = 2.0           # 1.5   # cart mass
m_p1 = 0.5          # 0.4   # 1st pole mass
m_p2 = 0.25         # 0.2   # 2nd pole mass
b_c = 1.0           # 0.5   # cart viskotic friction coefficient
b_p1 = 0.15                 # 1st pole viskotic friction coefficient
b_p2 = 0.1                  # 2nd pole viskotic friction coefficient
L_p1 = 1.2          #0.8    # 1st pole full length
L_p2 = 0.8          #0.4    # 2nd pole full length
l_1 = L_p1 / 2.0            # 1st pole "effective" length (from cart to mass center point)
l_2 = L_p2 / 2.0            # 2nd pole "effective" length (from cart to mass center point)
...
param_setup={'gravity': g, 'mass_cart': m_c, 'mass_pole_1': m_p1, 'mass_pole_2': m_p2, \
                                                ...
                                                'len_pole_1': L_p1, 'len_pole_2': L_p2, 'sampling': Ts, \
                                                'force_lim': max_force}
...
# Linearized matrices definition
# mass matrix
m11 = m_total
m12 = m_p1 * l_1
m13 = m_p2 * l_2
m21 = m12
m22 = 4.0 * m_p1 * (l_1**2)/3.0
m23 = 0
m31 = m13
m32 = m23
m33 = 4.0 * m_p2 * (l_2**2)/3.0
...
# concatenating linearized matrices
A_21 = M_0_inv @ G_0
A_22 = -M_0_inv @ F_0
...
if friction_on:
    ...
    # Q = np.eye(6)
    # np.fill_diagonal(Q, [215, 0.1, 0.1, 1, 1, 1])

    R = np.array([0.1], ndmin=2)
    Q = np.eye(6)
    np.fill_diagonal(Q, [922.0315571914417, 13.799534769332135, 0.01, 0.01, 1791.9920160380293, 0.01])
else:
    R = np.array([10], ndmin=2)
    Q = np.eye(6)
    np.fill_diagonal(Q, [1, 1, 1, 0, 0, 0])

# controller instance definition
lqr_ctrl = LQR_controller(6, 1)
lqr_ctrl.setup_ctrl(Q, R, A_lqr, B_lqr)

# environment instance definition
env = gym.make('ParallelInvPend-v1', init_setup=(10, 'heun', 'lqr'), param_setup=param_setup)
...
# starting state vector definition
init_state = np.array([0, 0, 0, 0, 0, 0.01]) # RL comparison
obs_state = env.reset()
...
for t in range(sim_length):
    # REFERENCE VECTOR CHANGES - UNCOMMENT WHEN CHECKING POSITIONING CONTROL
    # if t == np.ceil(sim_length/3):
    #     ref_k = np.array([1, 0, 0, 0, 0, 0], ndmin=2).T
    # elif t == np.ceil(2*sim_length/3):
    #     ref_k = np.array([-0.5, 0, 0, 0, 0, 0], ndmin=2).T
    
    # graphical rendering
    env.render()
    # state need to be adjusted to acceptable format - differences between model state vector and controller state vector
    ...
    # obs_state, reward, done, _ = env.step([0])

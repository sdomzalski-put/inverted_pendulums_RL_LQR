...
class DoubleInvPendV1(gym.Env):
    ...
    2) mass of a cart
    3) mass of a pole / rod 1
    4) mass of a pole / rod 2
    5) friction coefficient for cart linear movement
    6) friction coefficient for pendulum 1 rotational movement
    7) friction coefficient for pendulum 2 rotational movement
    8) length of pole 1
    9) length of pole 2
    10) inertia of pole 1
    11) inertia of pole 2
    12) system sampling time
    13) force limits on cart
    
    '''
    # def __init__(self, init_setup, param_setup=(9.81, 1.0, 0.1, 0.5, 0.02, 15.0, 0.01, 0.01)):
    def __init__(self, init_setup, param_setup={'gravity': 9.81, 'mass_cart': 0.603, 'mass_pole_1': 0.123, 'mass_pole_2': 0.092, \
                                                'b_cart': 2.21, 'b_pole_1': 0.005, 'b_pole_2': 0.005, \
                                                'len_pole_1': 0.49, 'len_pole_2': 0.49, \
                                                'inertia_pole_1': 0.005, 'inertia_pole_2': 0.003, \
                                                'sampling': 0.01, 'force_lim': 50.0}):
        ...
        assert len(param_setup) == 13

        # setting physics parameters

        self.gravity = param_setup['gravity']
        self.m_cart = param_setup['mass_cart']
        ...
        self.lengh_p1 = param_setup['len_pole_1']
        self.lengh_p2 = param_setup['len_pole_2']
        self.inertia_p1 = param_setup['inertia_pole_1']
        self.inertia_p2 = param_setup['inertia_pole_2']
        
        self.m_total = self.m_cart + self.m_p1 + self.m_p2
        self.l_1 = self.lengh_p1 / 2.0
        self.l_2 = self.lengh_p2 / 2.0
        ...
        # limit array - [x, dx, theta1, dtheta1, theta2, dtheta2]
    ...
    def step(self, f):
        ...
        costheta21 = math.cos(theta2 - theta1)
        sintheta21 = math.sin(theta2 - theta1)
        
        m11 = self.m_total
        m12 = (self.m_p1*self.l_1 + self.m_p2*self.lengh_p1)*costheta1
        m13 = self.m_p2*self.l_2*costheta2
        m21 = m12
        m22 = self.inertia_p1 + self.m_p1*(self.l_1**2) + self.m_p2*(self.lengh_p1**2)
        m23 = self.m_p2*self.lengh_p1*self.l_2*costheta21
        m31 = m13
        m32 = m23
        m33 = self.inertia_p2 + self.m_p2*(self.l_2**2)
        
        M = np.array([[m11, m12, m13], [m21, m22, m23], [m31, m32, m33]])
        M_inv = np.linalg.inv(M)
        
        f11 = self.b_cart
        f12 = -(self.m_p1*self.l_1 + self.m_p2*self.lengh_p1)*sintheta1*theta_dot1
        f13 = -self.m_p2*self.l_2*sintheta2*theta2
        f21 = 0
        f22 = self.b_p1
        f23 = -self.m_p2*self.lengh_p1*self.l_2*sintheta21*theta_dot2
        f31 = 0
        f32 = self.m_p2*self.lengh_p1*self.l_2*costheta21*theta1 - self.b_p2
        f33 = self.b_p2
        ...
        g1 = force
        g2 = (self.m_p1*self.l_1 + self.m_p2*self.lengh_p1)*self.gravity*sintheta1
        ...
        if self.control_algorithm == 'ql':
            ...
            # reward = -(theta/self.theta_up) ** 2 *( 1+ 1* (abs(x_dot/7))+ 1* (abs(x/self.x_threshold)))+ 1* (abs(x_dot/7))+ 1* (abs(x/self.x_threshold))
            reward = 2
    ...
    def render(self, mode='human'):
        ...
        polewidth = 10.0
        polelen1 = scale * self.lengh_p1
        polelen2 = scale * self.lengh_p2
        cartwidth = 50.0
        cartheight = 30.0
        ...
        if self.viewer is None:
            ...
            self.poletrans2 = rendering.Transform(translation=(0, axleoffset+polelen1))
        ...
        self.poletrans2.set_translation((polelen1-axleoffset)*math.sin(x[2]), axleoffset+(polelen1-axleoffset)*(math.cos(-x[2])))
